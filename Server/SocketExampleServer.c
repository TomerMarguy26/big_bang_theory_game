...
BOOL someone_inside = FALSE, lets_play = FALSE, an_error_occurred_remove_LB = FALSE;
...
int MainServer(char* server_port)
{
	...
	BOOL an_error_occurred_main = FALSE;
	...
	{
		an_error_occurred_main = TRUE;
		printf("An error has occurred while creating the mutex, Last Error = 0x%x\n!", GetLastError());
		goto cleanup_wsaup;
	}
	...
	{
		an_error_occurred_main = TRUE;
		printf("An error has occurred while creating the semaphore, Last Error = 0x%x\n!", GetLastError());
   ...
   */
	...
	*/
	...
	while (1) //endless clients, just 2 can connect
	{
		...
		an_error_occurred_main = FindFirstUnusedThreadSlot();
...
cleanup_wsaup:
	...
	if (an_error_occurred_main)
		if (an_error_occurred_remove_LB)
		return ERROR_CODE;
	return SUCCESS_CODE;
}
...
{
	BOOL an_error_occurred_to_main = FALSE;
	for (Ind = 0; Ind < NUM_OF_WORKER_THREADS; Ind++)
	{
		...
		else
		{
			...
			if (Res == WAIT_OBJECT_0) // this thread finished running
			{
				...
				{
					printf("Error when closing thread: %d\n", GetLastError());
					an_error_occurred_to_main = TRUE;
				}
				ThreadHandles[Ind] = NULL;
				break;
			}
	...
	return an_error_occurred_to_main;
}
...
{
	int Ind;
	BOOL an_error_occurred_cu = FALSE;
...
static DWORD ServiceThread(SOCKET* t_socket)
{
	...
	while (!Done)
	{
		BOOL an_error_occurred_service = FALSE;
		...
		//	int last_size = 0;
			...
			{
				an_error_occurred_service = TRUE;
		...
		else if (STRINGS_ARE_EQUAL(AcceptedStr, "CLIENT_PLAYER_MOVE"))
		{
			...
			else if (client_play)
			{
				...
				if (NULL == (fp_GameSession = fopen("GameSession.txt", "r")))//file is not exist, need to create! this is the first user to arrive here
				{
					...
					{
						printf("An error has occurred while deleting the file\n");
						goto socket_error;
					}
				}
...
int remove_leaderboard_and_rename_temp(BOOL first)
{
	...
	{
		an_error_occurred_remove_LB = TRUE;
		printf("An error has occurred while deleting the file\n");
		return ERROR_CODE;
	}
